...
class FromToPipeline:
    ...
    def __generate_node(self, fmpath, From, tos, verbose = False):
        #WIP
        for to in tos:
            path = fmpath + '/' + From + '/' + to
            ...
            self.__convert_raw_to_stops_format(raw, path)

    def __convert_raw_to_stops_format(self, raw, path):       
        From = raw.get('from')
        To = raw.get('to')
    ...
    def generate_map(self, name, overwrite = False):
        ...
        if overwrite: #or not os.path.exists(fmpath):
            ...
            for k, v in nodes.items():
                self.__generate_node(fmpath, k, v)
    #refactor
    def Read(self, force):
        #and casts to json
